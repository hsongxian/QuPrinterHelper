...
object PrinterHitiQueue826Old {
    ...
    fun doPrintWithHiti(printLists: ArrayList<PrintBean>, printerType: String) {
    ...
    private fun setPrintData() {
        ...
        if (errorCode == ErrorCode.ERR_CODE_SUCCESS) {
            if (!status.contains("Success")) {
                QuPrinter.sendPrintResultMsg(status, ResponsePrinter.PRINTER_ERROR, printerType)
                return
            }
            ...
            LogUtils.i("启动服务错误")
            QuPrinter.sendPrintResultMsg(errorCode.toString(), ResponsePrinter.PRINTER_ERROR, printerType)
        }
    }
    ...
    fun showResponse(job: PrinterJob) {
        ...
        LogUtils.i("${printerType}打印结果：$data")
        if (data.contains("0x0 Success")) {
            ...
            QuPrinter.sendPrintResultMsg(data, ResponsePrinter.PRINTER_ERROR, printerType)
        }
    }
